...
**The whole project in one line.** A self-hosted AI dungeon RPG: one local LLM plays every role through purpose-built contexts (agents); whatever it wants to change the world must pass a validated tool into SQLite (the single source of truth); a FastAPI brain resolves one turn per request and pushes late media over SSE; the frontend is a no-build vanilla SPA; image and voice are optional local services.
...
| [`infra/`](infra/index.md) | docker, ports, volumes, profiles | the seven-service compose stack | [open](../index.html#infra) |
...
**In-repo authority.** These twins summarize the live code maps: `orchestrator/INDEX.md` (+ `app/{engine,tools,repo,integrate,providers}/INDEX.md`) and `frontend/INDEX.md`. When code and a twin disagree, the code (and those INDEX maps) win.
