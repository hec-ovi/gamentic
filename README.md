...
- Pull a character aside for a private word no one else hears. What they confide there sticks: characters keep their own memory of what they shared with you, and their profile (past, traits, pivotal moments, image memories) grows out of how you treat them. Hand someone a gift and they answer you there, in private. Characters can whisper to you first, too, and whispers you have not read yet wait as a badge on their card.
...
**Image** (optional) is FLUX.2 [klein] 4B distilled in ComfyUI behind a small REST adapter: scene art, a 3-view reference set per character, identity-conditioned story shots, item cards. At creation an art-director pass reads the whole world bible and writes the first-sight prompts: every character's reference look first, then the main opening image. The model set is the Comfy-Org repack (`flux-2-klein-4b` + `qwen_3_4b` encoder + `flux2-vae`, about 16 GB). The game is fully playable text-only; art fills in as it renders.
